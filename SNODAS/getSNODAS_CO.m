...
for n=4:5; % get 2007
    ...
    for m=1:length(dm) % loop over month
        ...
        for p=1:length(d3) % loop over day
            ...
            delete('*.gz');
            dd=d3(1).name(end-5:end-4); % get day of month from file
            S2=[dd '-' S(4:6) '-' Snodas(n2).year];
            Snodas(n2).date(q)=datenum(S2);
            q=q+1;
        end
        cd(mw,'..');
    end  
    % convert all mass to units of [m]
    Snodas(nY).Precip=Snodas(nY).Precip/1000/10; % put in [m]
    Snodas(nY).SnowPrecip=Snodas(nY).SnowPrecip/1000/10; % snow accumulation [m WE]
    Snodas(nY).SWE=Snodas(nY).SWE/1000; % [m] SWE
    Snodas(nY).Depth=Snodas(nY).Depth/1000; % [m] Depth
    Snodas(nY).SublimationBS=Snodas(nY).SublimationBS/1e5; % [m] sublimation from blowing snow
    Snodas(nY).Sublimation=Snodas(nY).Sublimation/1e5; % [m] sublimation from snowpack
    Snodas(nY).Melt=Snodas(nY).Melt/1e5; % [m] melt
    nY=nY+1;
end
