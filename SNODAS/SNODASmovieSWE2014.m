...
ISx=find(strncmp(Pstate,{R.r.name}',3)); % find the SWE data
...
for n4=1:length(MonthD) % loop over months
    ...
    for n5=1:length(jx) % loop over days
        %try
            try R=load([YY '/' MonthD{n4} '/' D(jx(n5)).name]); % load current
                s{q}=[YY '/' MonthD{n4} '/' D(jx(n5)).name];
            catch
                disp('load error')
            end
