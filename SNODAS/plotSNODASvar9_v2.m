function G=plotSNODASvar9(LAT,LON,S,latlim,lonlim,zlim)
h=usamap(latlim, lonlim);
set(h,'nextplot','replacechildren');
Amap=ones(size(S)); Amap(S==0)=0; Amap(isnan(S))=0;
G=geoshow(LAT,LON,S,'displaytype','texturemap'); 
caxis(zlim)
hC=colorbar('location','south')
set(hC,'FontSize',14,'FontWeight','bold')
...
%%
hold on
load ~/HP_DRIVE/SnowEx2019/SnowSiteData/SNOTEL/SNOTEL.mat
Slon=[r.lon]; Slat=[r.lat];
Ix=find(Slon>lonlim(1) & Slon<lonlim(2) & Slat>latlim(1) & Slat<latlim(2));
r3=r(Ix);
for n=1:length(r3)
