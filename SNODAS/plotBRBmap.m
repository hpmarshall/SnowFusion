function plotBRBmap(latM,lonM,M)
% input latitude, longitude, and matrix of values will be rendered on elevation surface next to orthophoto.
% latM=[M,1] column vector of latitudes, lonM=[1,N] row vector of longitude, M=gridded data
% adapted from getDEMortho
...
[n3,m3]=size(Z);
latO=linspace(latlim(1),latlim(2),n3);
lonO=linspace(lonlim(1),lonlim(2),m3);
%[LONO,LATO]=meshgrid(lonO,latO); % make matricies for interpolation
%[LONM,LATM]=meshgrid(lonM,latM);
M2=interp2(lonM(:)',latM(:),M,lonO(:)',latO(:));
...
for n8=1:2
    ...
    if n8<2
        ...
        geoshow(Z, R, 'DisplayType', 'surface', 'CData', M2); % input data M interpolated to elevation grid
        %Z3=ones(size(M2)); Ix=find(M2==0); Z3(Ix)=0;
        %alpha(Z3);
    end
