...
MI=zeros(length(lat),length(lon));
for n=1:length(S2)
    Snodas(1).(S2{n})=single(MI*NaN); % initialize each output
%    Snodas(2).(S2{n})=single(MI*NaN); % initialize each output
end
...
nY=1;
%%
for n=7:7 %4:5; % loop over year
    q=1;
    D(n).name
    cd(mw,['/DATASETS/NOAA/G02158/masked/' D(n).name]);
    Snodas(nY).year=D(n).name;
    dm=dir(mw)
    for m=3:3 %1:length(dm) % loop over month
        Smonth=dm(m).name
        cd(mw,Smonth);
        d3=dir(mw,'*.tar');
        for p=19:19 %:length(d3) % loop over day
            mget(mw,d3(p).name);
            untar(d3(p).name);
            for p2=1:length(S) % loop over state (3=SWE)
                ...
                if ~isempty(Dt)
                    ...
                    Snodas(nY).(S2{p2})(:,:,q)=D3; % get area of interest
                    fclose(fid);
