...
Svars=4; % variables to plot
WY=2014; % Water year to make a movie for  
...
[i2,j2]=size(Svars); 
dx=1/j2;dy=0.95/i2;
hs=zeros(i2,j2); % matrix of handles
G=zeros(i2,j2);
for n2=1:i2
    ys=(n2-1)*dy;
    for n3=1:j2
        xs=(n3-1)*dx;
        hs(n2,n3)=subplot('position',[xs,ys+0.05,dx,dy]);
        St=R.r(Svars(n2,n3)).data(Ix,Iy)';
        St=full(St);
        hG(n2,n3)=plotSNODASvar9(LAT,LON,St,latlim,lonlim);
    end
end
figure(hfig)
%drawnow
M(1)=getframe(hfig);
hT=suptitle('2014-10-01');
q=2;
