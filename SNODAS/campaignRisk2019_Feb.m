...
for q=1:length(YR)
    q
    n=1;
    R=load([num2str(YR(q)) '/' Month '/S' num2str(YR(q)) '-' Month(2) '-' num2str(dd(n)) '.mat']); % load Mar date
    ...
    for n=1:12
        try
            ...
            disp(['missing file: ' num2str(YR(q)) '/' Month '/S' num2str(YR(q)) '-' Month(2) '-' num2str(dd(n)) '.mat'])
        end
    end
    MarSWE2(:,:,q)=median(MarSWE,3); % get median SWE for each pixel
    MarTemp2(:,:,q)=max(MarMelt,[],3); % sum all the melt during this period
end
save('Feb5GM_15yrs.mat','MarSWE2','MarTemp2','YR','lon','lat')
